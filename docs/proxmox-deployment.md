...
```
VLAN 10: Management (10.10.10.0/23)
...
| Storage | Type | Free Space | Usage |
|---------|------|------------|-------|
| ssd1 | SSD | 100GB | Core services (cloned from template) |
| vmdata | ZFS Pool | 2TB | Templates + Session containers - supports templates & deduplication |
...
# Create base container on vmdata (ZFS supports templates)
...
### Dashboard (CT 103)
```bash
pct clone 9000 103 --hostname dashboard --full
pct set 103 --memory 512 --cores 1 --net0 name=eth0,bridge=vmbr0,ip=10.10.10.102/24,gw=10.10.10.1
pct start 103
pct enter 103

# Configure
cat > /opt/queue-sim/.env << 'EOF'
REDIS_URL=redis://10.10.10.101:6379
DASHBOARD_PORT=8080
PROXMOX_HOST=10.10.10.10:8006
PROXMOX_USER=opnvdi@pam
PROXMOX_TOKEN_ID=UI2
PROXMOX_TOKEN_SECRET=33f7e112-ab02-42d5-9980-aa76eea2535d
EOF

# Start with PM2
cd /opt/queue-sim
pm2 start src/dashboard/server.ts --name dashboard --interpreter tsx
pm2 save
pm2 startup
```
...
### Gateway (CT 110)
```bash
pct clone 9000 111 --hostname gateway --full
pct set 111 --memory 1024 --cores 2 --net0 name=eth0,bridge=vmbr0,ip=10.10.10.111/24,gw=10.10.10.1
pct start 111
pct enter 111
...
cd /opt/queue-sim
pm2 start src/gateway/server.ts --name gateway --interpreter tsx
pm2 save
pm2 startup
```
...
for i in 120 121 122; do
  pct exec $i -- bash -c "cat > /opt/queue-sim/.env << 'EOF'
REDIS_URL=redis://:your-secure-password@10.10.10.101:6379
MATCHMAKER_ID=matchmaker-$i
EOF"
  pct exec $i -- bash -c "cd /opt/queue-sim && pm2 start src/matchmaker/worker.ts --name matchmaker --interpreter tsx && pm2 save && pm2 startup"
done
```
...
cd /opt/queue-sim
pm2 start src/scripts/proxmox_autoscaler.ts --name autoscaler --interpreter tsx
pm2 save
pm2 startup
```
...
```bash
# Clone to vmdata (ZFS) - sessions benefit from deduplication with 300 clones
pct clone 9000 9001 --hostname session-template --full --storage vmdata
...
cat > /opt/queue-sim/start-session.sh << 'EOF'
#!/bin/bash
cd /opt/queue-sim
export SESSION_ID=$(hostname | sed 's/session-//')
pm2 start src/session/runner.ts --name session --interpreter tsx -- --id=$SESSION_ID
EOF
chmod +x /opt/queue-sim/start-session.sh

# Add to rc.local
echo '/opt/queue-sim/start-session.sh' >> /etc/rc.local
